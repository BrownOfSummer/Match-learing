...
if __name__ == '__main__':
    ...
    for dset in ['train']:
        ...
        allclasses = {}
        fs = [open(os.path.join(_dest_set_dir, cls + '_' + dset + '.txt'), 'w') for cls in class_sets] # text_train.txt, dontcare_train.txt
        ftrain = open(os.path.join(_dest_set_dir, dset + '.txt'), 'w') #Main/train.txt

        files = glob.glob(os.path.join(_labeldir, '*.txt')) # label_tmp/*.txt
        files.sort()
        for file in files:
            path, basename = os.path.split(file) #path=/path/to/label_tmp, basename=out.txt
            stem, ext = os.path.splitext(basename) #stem=out, ext=.txt
